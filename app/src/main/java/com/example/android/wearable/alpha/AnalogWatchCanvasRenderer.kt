...
class AnalogWatchCanvasRenderer(
    ...
    private val clockInternalHandPaint = Paint().apply {
        isAntiAlias = true
        strokeWidth = context.resources.getDimensionPixelSize(R.dimen.clock_hand_stroke_width).toFloat()
    ...
    private lateinit var internalHourHandFill: Path
    private lateinit var internalHourHandBorder : Path
    private lateinit var minuteHandFill: Path
    private lateinit var minuteHandBorder: Path
    private lateinit var internalMinuteHandFill : Path
    private lateinit var internalMinuteHandBorder : Path
    ...
    private fun drawClockHands(
        ...
        canvas.withScale(
            ...
            withRotation(hourRotation, bounds.exactCenterX(), bounds.exactCenterY()) {
                if (drawAmbient) {
                    drawPath(internalHourHandBorder, clockInternalHandPaint)
                } else {
            ...
            withRotation(minuteRotation, bounds.exactCenterX(), bounds.exactCenterY()) {
                if (drawAmbient) {
                    drawPath(internalMinuteHandBorder, clockInternalHandPaint)
                } else {
    ...
    private fun recalculateClockHands(bounds: Rect) {
        ...
        internalHourHandBorder = createClockHand(
            bounds,
            watchFaceData.hourHandDimensions.lengthFraction - 0.12f,
            watchFaceData.hourHandDimensions.widthFraction - 0.02f,
            watchFaceData.gapBetweenHandAndCenterFraction + 0.1f,
            watchFaceData.hourHandDimensions.xRadiusRoundedCorners,
            watchFaceData.hourHandDimensions.yRadiusRoundedCorners
        )

        internalHourHandFill = internalHourHandBorder
        ...
        internalMinuteHandBorder =
            createClockHand(
                bounds,
                watchFaceData.minuteHandDimensions.lengthFraction - 0.22f,
                watchFaceData.minuteHandDimensions.widthFraction -0.02f,
                watchFaceData.gapBetweenHandAndCenterFraction + 0.2f,
                watchFaceData.minuteHandDimensions.xRadiusRoundedCorners,
                watchFaceData.minuteHandDimensions.yRadiusRoundedCorners
            )
        internalMinuteHandFill = internalMinuteHandBorder
    ...
    private fun drawCenterCircle(canvas: Canvas, bounds: Rect) {
        drawTopMiddleCircle(canvas, bounds, 0.025f, 0.475f)
    }
...
//                )
            //}
            val path = Path()
            path.addRect(bounds.exactCenterX()-0.001f*bounds.width(), 0.0f,
                bounds.exactCenterX()+0.001f*bounds.width(),0.08f*bounds.width(),Path.Direction.CW)
            canvas.drawPath(path, outerElementPaint)
